...
if (!TESTINY_API) throw new Error('TESTINY_API_URL is missing');
if (!API_KEY) throw new Error('TESTINY_API_KEY is missing');

export class TestinyService {
  ...
  private lastDebug: {
    attempts: Array<{ method: string; path: string; status?: number; error?: string }>;
    finalDataKeys?: string[];
    stepDiscoveryPath?: string;
    sampleStepText?: string;
  } = { attempts: [] };
  ...
  async getTestCaseRaw(id: string): Promise<any> {
    try {
      ...
      throw new Error(
        `Failed to fetch test case ${id}: ${message}. Detail: ${detail}. Base: ${TESTINY_API}. Attempted: ${attempted.join(', ')}`
      );
    }
  }

  async getTestCase(id: string): Promise<ManualTestCase> {
    ...
    if (discoveryPath) this.lastDebug.stepDiscoveryPath = discoveryPath;
    if (sampleStep) this.lastDebug.sampleStepText = sampleStep;
  ...
  private extractStepsAndExpected(payload: any): {
    steps: string[];
    expectedFromSteps: string[];
    ...
    const slateCandidates: Array<{ key: string; value: unknown }> = [
    ...
    const safeParseJson = (str: unknown): any | undefined => {
    ...
    const collectTextFromSlateNode = (node: any): string => {
      if (!node) return '';
      if (typeof node === 'string') return node;
      if (Array.isArray(node)) return node.map(collectTextFromSlateNode).join(' ').trim();
    ...
    const parseSlateForStepsExpected = (root: any): boolean => {
      ...
      const walkSlate = (n: any) => {
        if (!n || typeof n !== 'object') return;
        if (n.t === 'tr') rows.push(n);
      ...
      for (const row of rows) {
        const cells = (Array.isArray(row.c) ? row.c : Array.isArray(row.children) ? row.children : []).filter(
          (n: any) => n && (n.t === 'td' || n.t === 'th')
        );
        if (cells.length >= 1) {
        ...
        if (cells.length >= 2) {
          ...
          if (expText) expectedLines.push(`Expected: ${expText}`);
        }
      }
    ...
    for (const { key, value } of slateCandidates) {
      ...
      if (parsed) {
        ...
        if (used && !foundPath) foundPath = `${key}:slate-table`;
      }
    }
    ...
    const normalizeText = (value: unknown): string | undefined => {
      ...
      if (typeof value === 'object') {
        ...
        const maybe =
          ...
          (value as any).value;
        if (typeof maybe === 'string') return normalizeText(maybe);
      }
      return undefined;
    };

    const pushFromStepObject = (obj: any) => {
      const text =
        normalizeText(obj?.action) ||
      ...
      if (expected) expectedLines.push(`Expected: ${expected}`);
    };

    const isStepArray = (arr: any[]): boolean => {
    ...
    const walk = (node: any, path: string[] = []) => {
      ...
      if (Array.isArray(node) && isStepArray(node)) {
        ...
        if (!foundPath) foundPath = path.join('.') || '(root-array)';
        return;
      }
        ...
        if (containersToCheck.includes(key)) {
          if (Array.isArray(value)) {
              ...
              for (const section of value) {
                if (Array.isArray(section?.steps)) {
                  if (isStepArray(section.steps)) {
                    ...
                    if (!foundPath) foundPath = [...path, key, 'steps'].join('.');
                  }
                }
                if (Array.isArray(section?.children)) {
                  for (const child of section.children) {
                    if (Array.isArray(child?.steps) && isStepArray(child.steps)) {
                      ...
                      if (!foundPath) foundPath = [...path, key, 'children', 'steps'].join('.');
                    }
                  }
                }
              }
            ...
            if (isStepArray(value)) {
              ...
              if (!foundPath) foundPath = [...path, key].join('.');
            }
      ...
      for (const [k, v] of Object.entries(node)) {
        if (Array.isArray(v) && isStepArray(v)) {
          ...
          if (!foundPath) foundPath = [...path, k].join('.');
    ...
    const pushPreconditions = (val: unknown) => {
    ...
    if (payload?.preconditions) pushPreconditions(payload.preconditions);
    if (payload?.precondition) pushPreconditions(payload.precondition);
    ...
    if (stepTexts.length === 0) {
      const longTextCandidate =
        payload?.procedure || payload?.howto || payload?.description || payload?.content || payload?.text;
      ...
      if (normalized) {
        ...
        if (!foundPath) foundPath = 'fallback:description-split';
      }
    }
