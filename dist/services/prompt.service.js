...
class PromptService {
    generatePrompt(tc, target = 'pytest-appium-ios') {
...
- Use helpers: click_by_text, click_any_by_text, wait_for_text, wait_not_present, enter_text_by_label, scroll_to_text, tap_system_alert, select_first_photo.
...
`.trim();
    ...
    generateSimpleTest(tc, target = 'pytest-appium-ios') {
        const title = tc.title.replace(/'/g, "\\'");
        if (target === 'appium-js') {
            const stepsBlock = tc.steps
                .map((s, i) => `  // Step ${i + 1}: ${s}` + (tc.expectedPerStep?.[i] ? `\n  // ${tc.expectedPerStep?.[i]}` : ''))
...
`;
        ...
        if (target === 'pytest-appium-ios') {
            ...
            const buildActionForStep = (s, idx) => {
